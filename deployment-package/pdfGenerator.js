    ...
    try {
      ...
      for (let i = 0; i < vouchers.length; i++) {
        ...
        if (hasPassport) {
          ...
          // Registration instructions - "Scan or visit" text
          doc.fontSize(9)
             .fillColor('#666666')
             .font('Helvetica')
             .text('Scan QR code or visit:', margin, yPos, { width: contentWidth, align: 'center' });

          yPos += 15;

          // Registration URL (centered, on its own line)
          doc.fontSize(9)
             .fillColor('#2196F3')
             .font('Helvetica')
...
const generateThermalReceiptPDF = async (voucher) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const chunks = [];
      doc.on('data', chunk => {
        console.log('[Thermal Receipt] Data chunk received, size:', chunk.length);
        chunks.push(chunk);
      });
      doc.on('end', () => {
        const buffer = Buffer.concat(chunks);
        console.log('[Thermal Receipt] PDF generation complete, buffer size:', buffer.length);
        resolve(buffer);
      });
      doc.on('error', (err) => {
        console.error('[Thermal Receipt] PDF stream error:', err);
        reject(err);
      });
      ...
      console.error('[Thermal Receipt] Generation error:', err.message);
      console.error('[Thermal Receipt] Stack:', err.stack);
      reject(err);
    }
  });
};

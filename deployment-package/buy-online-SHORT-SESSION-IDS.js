 ...
 * REVERTED to original format for BSP DOKU compatibility
 * Format: PGKO-{timestamp}-{random}
 * @returns {string} Session ID (e.g., PGKO-1737164732561-X3KJ9P2L7)
 *
 * ⚠️ CRITICAL FIX: Reverted to original simple alphanumeric format
 * - NO base64url encoding (removes special chars like - and _)
 * - Uses Math.random() with base36 encoding (0-9, A-Z only)
 * - BSP DOKU appears to reject base64url characters in session IDs
 * - Format: PGKO-{milliseconds}-{9 uppercase alphanumeric}
 */

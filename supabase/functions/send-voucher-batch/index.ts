...
async function generateBatchHTML(batch: BatchData, vouchers: VoucherData[]): Promise<string> {
...
<head>
  <meta charset="UTF-8">
  <title>PNG Green Fees - Batch ${batch.batch_number}</title>
  <style>
    body {
...
<body>
  <div class="header">
    ...
    <p style="color: #6b7280; margin: 5px 0;">Bulk Upload Batch Export</p>
  ...
  <div class="summary">
    ...
    <p><strong>Original File:</strong> ${batch.filename}</p>
    <p><strong>Total Records:</strong> ${batch.total_records}</p>
    <p><strong>Successful:</strong> ${batch.successful_records}</p>
    <p><strong>Failed:</strong> ${batch.failed_records}</p>
...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (!batchId || !email) {
      return json({ error: "batchId and email are required" }, 400);
    }
    ...
    if (batchError || !batch) {
      return json({ error: "Batch not found", details: batchError?.message }, 404);
    }

    // Fetch vouchers created from this batch
    const { data: purchases, error: purchasesError } = await supabase
      ...
      `)
      .eq("bulk_upload_id", batchId);

    if (purchasesError) {
      return json({ error: "Failed to fetch vouchers", details: purchasesError.message }, 500);
    }

    if (!purchases || purchases.length === 0) {
      return json({ error: "No vouchers found for this batch" }, 404);
    }
    ...
    const batchHtml = await generateBatchHTML(batch, vouchers);

    // Create email HTML
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #10b981;">PNG Green Fees - Batch Vouchers</h2>
        <p>Dear User,</p>
        <p>Please find attached the vouchers from batch <strong>${batch.batch_number}</strong>.</p>

        <div style="background: #f0fdf4; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #86efac;">
          <p style="margin: 5px 0;"><strong>Batch Number:</strong> ${batch.batch_number}</p>
          <p style="margin: 5px 0;"><strong>Original File:</strong> ${batch.filename}</p>
      ...
      <div style="margin-top: 40px; border-top: 2px solid #10b981; padding-top: 20px;">
        <h3 style="color: #10b981; text-align: center;">Batch Voucher Details</h3>
        ${batchHtml}
      </div>
    `;
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: email,
        subject: `PNG Green Fees - Batch ${batch.batch_number} (${vouchers.length} Vouchers)`,
        html: emailHtml,
        tags: [{ name: "type", value: "voucher-batch" }],
      }),
    });
    ...
    return json({
      ...
      message: `Batch ${batch.batch_number} with ${vouchers.length} voucher(s) sent to ${email}`
    });
  } catch (e) {
    console.error("send-voucher-batch error:", e);

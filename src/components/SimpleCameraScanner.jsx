...
const SimpleCameraScanner = ({ onScanSuccess, onClose }) => {
  ...
  const parseMRZ = (text) => {
    try {
      ...
      // Convert ISO 3166-1 alpha-3 nationality code to full country name
      const nationalityFullName = ISO_COUNTRY_CODES[nationality] || nationality;
      console.log('Nationality code:', nationality, '-> Full name:', nationalityFullName);

      const result = {
        passportNumber,
        surname,
        givenName,
        nationality: nationalityFullName,  // Use full country name instead of ISO code
        dateOfBirth,
        sex,
        dateOfExpiry
      };

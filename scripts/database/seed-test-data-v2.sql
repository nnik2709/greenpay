-- Seed Test Data for PNG Green Fees System (v2 - Corrected Table Names)
-- Run with:
-- PGPASSWORD='GreenPay2025!Secure#PG' psql -h localhost -U greenpay_user -d greenpay_db -f seed-test-data-v2.sql
...
-- Note: The actual table schema uses snake_case column names
...
-- ===========================================
-- 3. CREATE INVOICES (if table exists)
-- ===========================================
...
BEGIN
  ...
  IF quotation_id IS NOT NULL THEN
    ...
    BEGIN
      ...
      VALUES
        (quotation_id, 'INV-2025-' || LPAD(FLOOR(RANDOM() * 100000)::TEXT, 5, '0'), NOW() - INTERVAL '15 days', NOW() + INTERVAL '30 days', 6875.00, 0, 'unpaid', finance_manager_id, NOW() - INTERVAL '15 days', NOW() - INTERVAL '15 days')
      ON CONFLICT DO NOTHING;

      -- Update quotation status
      UPDATE "Quotation" SET status = 'approved', "updatedAt" = NOW() WHERE id = quotation_id;

      RAISE NOTICE 'Created unpaid invoice';
    EXCEPTION
      WHEN undefined_table THEN
        RAISE NOTICE 'Invoices table does not exist, skipping invoice creation';
      WHEN OTHERS THEN
        RAISE NOTICE 'Error creating invoice: %', SQLERRM;
    END;
  ELSE
    RAISE NOTICE 'No draft quotations found, skipping invoice creation';
  END IF;
END $$;
...
SELECT 'Passports:' AS category, COUNT(*) AS count FROM "Passport";
SELECT 'Individual Purchases:' AS category, COUNT(*) AS count FROM individual_purchases;

\echo ''
\echo 'Recent vouchers created:'
